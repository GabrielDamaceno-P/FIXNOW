...
// $_navDepth=0 → página em view/prestador/ | $_navDepth=1 → página em view/
$_navDepth = $_navDepth ?? 0;
$_prest    = $_navDepth === 1 ? 'prestador/' : '';   // prefixo para páginas do prestador
$_root     = $_navDepth === 1 ? '../'        : '../../'; // raiz do projeto
$_view     = $_navDepth === 1 ? ''           : '../';    // diretório view/
?>
<nav class="navbar navbar-expand-lg navbar-dark bg-primary fixed-top shadow-sm">
  <div class="container">
    <a class="navbar-brand fw-bold" href="<?= $_prest ?>dashboardPrestador.php">Fix Now</a>
    ...
    <div class="collapse navbar-collapse" id="prestMenu">
      <ul class="navbar-nav me-auto">
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'chamados' ? 'active' : '' ?>" href="<?= $_prest ?>dashboardPrestador.php">
            <i class="bi bi-house me-1"></i>Chamados
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'calendario' ? 'active' : '' ?>" href="<?= $_prest ?>calendario.php">
            <i class="bi bi-calendar3 me-1"></i>Calendário
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'orcamentos' ? 'active' : '' ?>" href="<?= $_prest ?>orcamento.php">
            <i class="bi bi-file-earmark-text me-1"></i>Orçamentos
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'servicos' ? 'active' : '' ?>" href="<?= $_prest ?>servicos.php">
            <i class="bi bi-tools me-1"></i>Serviços
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'portfolio' ? 'active' : '' ?>" href="<?= $_prest ?>portfolio.php">
            <i class="bi bi-images me-1"></i>Portfólio
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= $paginaAtiva === 'financeiro' ? 'active' : '' ?>" href="<?= $_prest ?>financeiro.php">
      ...
      <ul class="navbar-nav ms-auto align-items-lg-center gap-1">
        <li class="nav-item">
          <a class="nav-link position-relative px-2" href="<?= $_view ?>notificacoes.php" title="Notificações">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle d-flex align-items-center gap-2 py-1" href="#" data-bs-toggle="dropdown" aria-expanded="false">
            <?php if ($tecnicoFoto): ?>
              <img src="<?= $_root ?><?= htmlspecialchars($tecnicoFoto) ?>" alt="Foto" width="32" height="32" class="rounded-circle border border-2 border-white border-opacity-50" style="object-fit:cover">
            <?php else: ?>
          ...
          <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0" style="min-width:180px">
            ...
            <li><a class="dropdown-item py-2" href="<?= $_view ?>perfil.php"><i class="bi bi-person me-2 text-primary"></i>Meu Perfil</a></li>
            <li><a class="dropdown-item py-2" href="<?= $_view ?>suporte.php"><i class="bi bi-headset me-2 text-primary"></i>Suporte</a></li>
            <li><hr class="dropdown-divider my-1"></li>
            <li><a class="dropdown-item py-2 text-danger" href="<?= $_root ?>logout.php?entidade=prestador"><i class="bi bi-box-arrow-right me-2"></i>Sair</a></li>
          </ul>
        </li>
      </ul>
    </div>
